...
const bloqueLabels: Record<string, string> = {
  B1: "Grupo", B2: "Núcleo", B3: "Clientes", B4: "Operativa",
};

const dificultadLabels: Record<string, { label: string; dot: string }> = {
  verde: { label: "Fácil / Interno", dot: "bg-emerald-500" },
  amarillo: { label: "Especialista", dot: "bg-amber-500" },
  rojo: { label: "Complejo", dot: "bg-red-500" },
};
        ...
        <div className="max-w-6xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-500">
              Mostrando <strong className="text-white">{filtered.length}</strong> de {data.length} ideas
            </span>
          </div>
        ...
        <div className="space-y-3">
                    ...
                    >
                      <div className="px-6 pb-5 pt-2 border-t border-slate-800 grid md:grid-cols-3 gap-6">
                        <div className="md:col-span-2">
                          <h4 className="text-xs text-slate-500 uppercase tracking-widest font-bold mb-2">Descripción</h4>
                          <p className="text-sm text-slate-300 leading-relaxed">{idea.descripcion}</p>
                        </div>
                        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                          <h4 className="text-xs text-slate-500 uppercase tracking-widest font-bold mb-2">Límites / Riesgos</h4>
                          ...
                          <div className="mt-3 pt-3 border-t border-slate-800 flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-500">
                              {idea.requiere_especialista ? "Requiere especialista externo" : "Implementable internamente"}
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20 text-slate-500">
              ...
              <p className="font-medium">No se encontraron ideas con esos filtros.</p>
            </div>
          )}
        </div>
      </div>
